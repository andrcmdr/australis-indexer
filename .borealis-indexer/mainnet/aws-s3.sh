...
./s5cmd -r 100 --numworkers 256 --no-sign-request cp --flatten --if-source-newer "s3://near-protocol-public/backups/mainnet/rpc/latest" ./aws-s3/ ;
./s5cmd -r 100 --numworkers 256 --no-sign-request ls "s3://near-protocol-public/backups/mainnet/rpc/$(cat ./aws-s3/latest)/*" > ./aws-s3.ls ;

sed -r -s -e "s/^([0-9]+\/[0-9]+\/[0-9]+\s+[0-9]+\:[0-9]+\:[0-9]+\s+[0-9]+\s+)/https\:\/\/near\-protocol\-public\.s3\.ca\-central\-1\.amazonaws\.com\/backups\/mainnet\/rpc\/$(cat ./aws-s3/latest)\//gI" ./aws-s3.ls > ./aws-s3.list ;
